...
import bcrypt
from .models import PropertyMaster,Property_TypeMaster,TypeMaster,Description,AvgMaster,RatioDetails
from django.db.models import Q, Count
from django.shortcuts import render, get_object_or_404
#from .serializers import JournalSerializer
import pandas as pd
...
def Alert(request):
    dfp = pd.read_csv("bckp.csv")
    df = pd.read_csv("FunCall2.csv.csv")
    # dfp = pd.read_csv("rishulife.csv")
    # df = pd.read_csv("core/Alert4.csv")
...
def Alert2(view_state,view_percentage):
    ...
    print("DF1", df1)
    print("ALert", df1)
    file_name = view_state + "_" + str(view_percentage) + ".csv"
...
def testemail(df1,file_name,a,b):
    # MailTester()
    r=RatioDetails.objects.all()
    for i in r :
        print("rks")
        print(i.percentage)
        print(i.state)
    ...
    email = EmailMessage('Cheap Land Search ' + ' ' + a + ' - ' + str(b), '', 'joshi.anshul2@gmail.com', ['joshi.kush500@gmail.com'])
    # for i in range(5) :
        # Alert2()
    email.attach_file(file_name)
    # email.attach_file(file_name)
    # email.attach_file('rishu777.csv')
...
def show(request):
    ...
    # df = pd.read_csv("rishulife.csv")
    # pd.df['created_at'].date()
    if request.method == 'POST':
        aj = request.POST.get('Descrpt')
        prop2 = PropertyMaster.objects.filter(id=id).update(Descrpt=aj)
        prop2.save()
...
def desp(request):
   ...
   if request.method == 'POST' :
      aj = request.POST.get('Descrpt')
      print(aj)
      one_entry = PropertyMaster.objects.get(id=id)
      prop2 = one_entry.objects.update(Descrpt=aj)
      prop2.save()
   a=Description.objects.all()
   context = {
       aj : a ,
   }
   # return HttpResponseRedirect("")
...
def is_valid_queryparam(param):
    return param != 'login' and param is not None
...
def disp(request,id):
    data = dict()
    msg = request.GET['Descrpt']
    print(msg)
    Data = PropertyMaster.objects.filter(lwPropertyId = id).update(Descrpt = msg)
    print(Data)
    data['msg'] = msg
