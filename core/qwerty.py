...
dataFrame =pds.read_sql("SELECT state,county, SUM(price)/SUM(acres) AS NETPRICE FROM core_propertymaster GROUP BY state,county",engine) 

conn = engine.raw_connection()
cur = conn.cursor()
output = io.StringIO()
dataFrame['netprice'] = dataFrame['netprice'].apply(np.round, args=[4])
...
conn.commit()
dataFrame.to_csv('FunCall2.csv',index=False)
